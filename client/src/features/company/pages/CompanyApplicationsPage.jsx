      ...
      {viewMode === 'board' ? (
        <div className="flex gap-4 overflow-x-auto pb-6 pt-2 snap-x">
          {KANBAN_STAGES.map((col) => {
            ...
            return (
              <div
                key={col.id}
                className="w-80 shrink-0 bg-slate-900/80 rounded-3xl border border-slate-800 p-4 flex flex-col justify-between shadow-xl min-h-[500px]"
              >
                {/* Column Header */}
        ...
        <div className="space-y-6">
          {groupedJobSections.map((group) => (
            <div key={group.jobId} className="glass-panel p-6 rounded-3xl border border-slate-800 bg-slate-900/90 space-y-4">
