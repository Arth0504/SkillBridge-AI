    ...
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      <div>
        <h1 className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight flex items-center gap-3">
          <MessageSquare className="w-8 h-8 text-brand-500" /> AI Mock Interview Simulator
        </h1>
      ...
      <div className="flex items-center gap-2 border-b border-slate-800 pb-2">
        ...
        >
          <Play className="w-4 h-4" /> Live Practice Room
        </button>
        <button
            ...
            <div className="space-y-6">
              ...
              <div className="glass-panel p-6 rounded-3xl flex justify-between items-center border border-slate-800">
                <div>
                  ...
                  <span className="text-xs text-slate-400 ml-3">Difficulty: {activeSession.difficulty || difficulty}</span>
                </div>
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass-panel p-8 rounded-3xl space-y-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="text-[11px] font-bold uppercase text-brand-400">Gemini AI Interviewer Question</span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-between items-center pt-2">
                    <Button variant="outline" size="sm" onClick={handleFinishSession} isLoading={finishSessionMutation.isPending}>
                      Generate Final Recruiter Report
                    </Button>
                    <Button variant="primary" isLoading={submitAnswerMutation.isPending} onClick={handleAnswerSubmit}>
              ...
              {activeSession.lastEvaluation && (
                <div className="glass-panel p-6 rounded-3xl border border-emerald-500/30 bg-emerald-500/5 space-y-3">
                  ...
                  <p className="text-xs text-slate-300 leading-relaxed">{activeSession.lastEvaluation.feedback}</p>
                </div>
              )}
            </div>
          )}
      ...
      {activeTab === 'history' && (
        <div className="space-y-4">
          {history.length === 0 ? (
            <p className="text-xs text-slate-400 italic">No past interview sessions logged.</p>
          ) : (
            history.map((session, idx) => (
              <div key={session._id || idx} className="glass-card p-6 rounded-3xl border border-slate-800 flex justify-between items-center">
                <div>
                  <div className="flex items-center gap-3">
                    <h4 className="text-sm font-bold text-white">{session.category || 'Interview Session'}</h4>
